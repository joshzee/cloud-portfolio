---
title: "Homelab Phase 3: Git Gets the Keys"
...
slug: "HL-phase-3-git-gets-the-keys"
tags:
  - homelab
  - kubernetes
  - k3s
...
Argo CD provides a prominent web interface as well as a CLI. Flux is more CLI- and Kubernetes-API-first and does not ship with an equivalent central dashboard. The course deliberately uses Flux so that I work directly with Git, manifests and reconciliation rather than learning the workflow mainly through a UI.
...
There is also some practical relevance to the mentor's Azure-focused work. Microsoft provides [Flux v2 as the `microsoft.flux` cluster extension](https://learn.microsoft.com/en-us/azure/azure-arc/kubernetes/conceptual-gitops-flux2) for both Azure Kubernetes Service and Azure Arc-enabled Kubernetes. Flux is not restricted to Azure—and Azure can also run Argo CD—but it has a clear first-party integration path there.
